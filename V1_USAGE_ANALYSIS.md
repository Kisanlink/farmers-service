...
## Overview
This document identifies all locations in the current codebase where the farmer module v1 services are being used. These locations need to be updated to use the new v2 farmer_service.
...
// NEW (v2)
import { farmerService, FarmerResponse } from '../farmer_service';
```
...
// NEW (v2)
const farmers = await farmerService.identity.listFarmers({ fpo_reg_no: fpoRegNo });
```
...
The migration will involve updating imports, method calls, and type definitions across the entire application to use the new v2 farmer_service.
